...
module("Integration | Component | share-button", function (hooks) {
  ...
  test("it supports passing @via", async function (assert) {
    await render(hbs`<ShareButton @via="ember.js">Tweet this!</ShareButton>`);
    assert.equal(this.tweetParam("via"), "ember.js");
  });

  test("it supports adding extra classes", async (assert) => {
    await render(
  ...
  test("the target, rel and href attributes cannot be overridden", async (assert) => {
    await render(
